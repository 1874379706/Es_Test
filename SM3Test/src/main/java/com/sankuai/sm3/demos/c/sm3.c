...
JNIEXPORT void JNICALL Java_SM3Util_sm3(JNIEnv *env, jobject obj, jbyteArray input, jint ilen, jbyteArray output) {
    jbyte *inputBytes = (*env)->GetByteArrayElements(env, input, NULL);
    jbyte *outputBytes = (*env)->GetByteArrayElements(env, output, NULL);

    // 调用SM3加密函数
    sm3((unsigned char *)inputBytes, ilen, (unsigned char *)outputBytes);
